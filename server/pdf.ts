...
export async function generatePDF(
  ...
  // Approval Section with proper page management
  checkPageBreak(35);
  ...
  if (type === 'requisition') {
    ...
    doc.setFont('helvetica', 'bold');
    doc.text(`${requisitionData.user.firstName || ''} ${requisitionData.user.lastName || ''}`, margin, currentY + 6);
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(8);
    doc.text(requisitionData.user.role || 'Requester', margin, currentY + 11);
    doc.text(format(new Date(requisitionData.requisition.requestDate), 'MMM dd, yyyy'), margin, currentY + 16);
    ...
    if (requisitionData.requisition.status === 'pending') {
      ...
      doc.text(`Pending Approval`, margin + 100, currentY + 6);
    } else if (requisitionData.requisition.status === 'approved') {
      doc.setFont('helvetica', 'bold');
      doc.text(`Finance Team`, margin + 100, currentY + 6);
    } else if (requisitionData.requisition.status === 'rejected') {
      doc.setFont('helvetica', 'bold');
      doc.setTextColor(220, 53, 69);
      doc.text(`Rejected`, margin + 100, currentY + 6);
      doc.setTextColor(0, 0, 0);
    }
  } else {
    const poData = data as PurchaseOrderData;
    ...
    doc.text(`Raised By:`, margin, currentY);
    doc.setFont('helvetica', 'bold');
    doc.text(`${poData.approver.firstName || ''} ${poData.approver.lastName || ''}`, margin, currentY + 6);
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(8);
    doc.text(poData.approver.role || 'Finance Team', margin, currentY + 11);
    doc.text(format(new Date(poData.purchaseOrder.issueDate), 'MMM dd, yyyy'), margin, currentY + 16);
    
    doc.setFontSize(10);
    doc.text(`Original Requisition:`, margin + 100, currentY);
    doc.setFont('helvetica', 'bold');
    doc.text(poData.requisition.requisitionNumber, margin + 100, currentY + 6);
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(8);
    doc.text(`Requested by: ${poData.requisition.requestedById}`, margin + 100, currentY + 11);
    doc.text(format(new Date(poData.requisition.requestDate), 'MMM dd, yyyy'), margin + 100, currentY + 16);
  }
  ...
  doc.text(`Generated: ${format(new Date(), 'MMM dd, yyyy h:mm a')}`, pageWidth - 70, currentY);
